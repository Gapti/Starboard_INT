...
#region MEnu
	...
	{
		pc_AboutWindow.Init("Assets/ProCore/QuickDecals/About/pc_AboutEntry_QuickDecals.txt", true);
	}
...
#region GUI
	...
	void OnGUI()
	{
		...
		toolbarRect = new Rect(0, isFloating ? -10 : 0, Screen.width, 28);
...
#region GUI Helpers
	...
	void PerformDragDrop(List<Decal> dec, int group, int index)
	{
		if(group > -1)
...
#region Decal Placement
	...
	bool PlaceDecal(Vector2 mousePosition, Material mat, Rect uvCoordinates, float rotation, float scale, out GameObject decal)
	{
		decal = null;
...
#region Decal Management
	...
	private Decal FirstSelectedDecal()
	{
